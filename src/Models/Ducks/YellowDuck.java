...
public class YellowDuck implements Duck {
    ...
    public YellowDuck(int y, int direction){
        this.y = y;
        if (direction == 0){
            this.x = 800;
            this.direction = "RIGHT";
            for (int i = 0; i < right.length; i++) {
                right[i] = MakeFrames.makeFrames("Files/Images/Ducks/YellowDuck.png", this.direction, i);
            }
        }
        else {
            this.x = -100;
            ...
            for (int i = 0; i < left.length; i++) {
                left[i] = MakeFrames.makeFrames("Files/Images/Ducks/YellowDuck.png", this.direction, i);
            }
        }
        this.HP = tempHP;
